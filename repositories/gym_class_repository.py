...
def save(gym_class):
    ...
    values = [gym_class.name, gym_class.date, gym_class.time,
              gym_class.capacity, gym_class.class_type]
...
def select(id):
    ...
    if result is not None:
        gym_class = Gym_Class(result['name'], result['date'], result['time'], result['capacity'],
                              result['class_type'], result['id'])
    return gym_class
...
def update(gym_class):
    ...
    values = [gym_class.name, gym_class.date, gym_class.time, gym_class.capacity,
              gym_class.class_type, gym_class.id]
    run_sql(sql, values)
...
def get_by_member(member):
    ...
    for row in results:
        gym_class = Gym_Class(row['name'], row['date'], row['time'],
                              row['capacity'], row['class_type'])
        gym_classes.append(gym_class)
...
def check_current_capacity(id):
    sql = "SELECT current_capacity FROM gym_classes WHERE id = %s"
    values = [id]
    results = run_sql(sql, values)

    return results

# def update_capacity(gym_class):
#     sql = "UPDATE gym_classes SET current_capacity = %s WHERE id = %s"
#     values = [gym_class.current_capacity, gym_class.id]
#     run_sql(sql, values)
